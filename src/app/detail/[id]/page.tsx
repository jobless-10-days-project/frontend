...
export default function ProfileDetail({ params }: { params: any }) {
    ...
    const tmpToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxIiwiaWF0IjoxNjk4NDg0MDQ5LCJleHAiOjE2OTg0ODc2NDl9.dbWKptonm5YnidEHBIW2E2zfiUAlpLHYhgke386tMnc"
    const id = params.id
    const star = [...Array(5)]
    ...
    // 6532043021
    useEffect(() => {
        axios.get(`${process.env.NEXT_PUBLIC_BACKEND_ENDPOINT}/user/stu/${id}`, { headers: headers }).then(data => {
        ...
        (
            <main className='mt-[4.3rem] w-[20rem] mx-auto'>
                ...
                <div className="ml-auto mr-auto mt-2  font-['Sarabun']">
                    ...
                    <p className='font-light text-xs my-1'> {`${details.price} bulbs / request`}</p>
                ...
                <div className='flex ml-4'>
                    ...
                    {
                        star.map((_, index) => {
                            if (index < details.averageScore) return (<div className='my-auto' key={index}><AiFillStar size={20} color='#EEBD5C' /></div>)
                ...
                <div className="bg-gray-100 rounded-xl p-5 flex-col font-['Sarabun']">
                    ...
                    {details.reviews.map((props, index) => (<Review key={index} {...props} />))}
                </div>
            </main>
        )
        :
